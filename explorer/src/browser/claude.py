...
class ClaudeInterface(BaseLLMInterface):
    ...
    async def start_new_chat(self):
        ...
        if self._response_in_progress:
            logger.warning("[claude] start_new_chat blocked: response in progress!")
            return

        try:
            logger.info("[claude] Starting new chat")
    ...
    async def enable_extended_thinking(self) -> bool:
        ...
        """
        if self._response_in_progress:
            logger.warning("[claude] enable_extended_thinking blocked: response in progress!")
            return self.extended_thinking_enabled

        if self.extended_thinking_enabled:
            logger.info("[claude] Extended Thinking already enabled, skipping")
            return True

        logger.info("[claude] Enabling Extended Thinking mode...")
        ...
        try:
            ...
            for selector in model_btn_selectors:
                try:
                    ...
                    if model_btn and await model_btn.is_visible():
                        logger.info(f"[claude] Found model selector: {selector}")
                        break
                    model_btn = None
                except:
                    continue
                ...
                for selector in thinking_options:
                    try:
                        ...
                        if option and await option.is_visible():
                            ...
                            logger.info(f"[claude] Selecting Extended Thinking: {text[:30]}")
                            await option.click()
            ...
            for selector in toggle_selectors:
                try:
                    ...
                    if toggle and await toggle.is_visible():
                        logger.info(f"[claude] Found thinking toggle: {selector}")
                        await toggle.click()
                        await asyncio.sleep(0.5)
                        self.extended_thinking_enabled = True
                        return True
            ...
            logger.warning("[claude] Extended Thinking toggle not found")
            return False

        except Exception as e:
            logger.error(f"[claude] Could not enable Extended Thinking: {e}")
            return False
    ...
    async def send_message(self, message: str, use_extended_thinking: bool = False) -> str:
        ...
        logger.info(f"[claude] Sending message in {mode_str} mode ({len(message)} chars)")
        ...
        try:
            ...
            logger.info(f"[claude] Response guard ON - waiting for response...")

            try:
                response = await self._wait_for_response()
            finally:
                self._response_in_progress = False
                logger.info(f"[claude] Response guard OFF")
    ...
    async def _wait_for_response(self, timeout: int = None) -> str:
        ...
        mode_str = "Extended Thinking" if self.extended_thinking_enabled else "standard"
        logger.info(f"[claude] Waiting for response in {mode_str} mode (timeout: {timeout}s)...")
        ...
        logger.info(f"[claude] Initial wait: {initial_wait}s before stability checks")
        ...
        while (datetime.now() - start_time).seconds < timeout:
            ...
            if (datetime.now() - last_log_time).seconds >= 30:
                logger.info(f"[claude] Still waiting... ({elapsed}s elapsed, stable_count={stable_count})")
            ...
            if current_response:
                if current_response == last_response:
                    ...
                    if stable_count >= required_stable:
                        ...
                        if not final_check:
                            elapsed = (datetime.now() - start_time).seconds
                            logger.info(f"[claude] Response complete ({elapsed}s, {len(current_response)} chars)")
        ...
        if last_response:
            elapsed = (datetime.now() - start_time).seconds
            logger.warning(f"[claude] Timeout reached after {elapsed}s, returning partial response")
        ...
        for _ in range(60):
            try:
                ...
                if "/chat/" in current_url and current_url != base_url:
                    logger.info(f"[claude] URL changed to conversation URL: {current_url}")
                    self._notify_url_change(current_url)
                    return
            except Exception as e:
                logger.debug(f"[claude] URL monitor error: {e}")
            await asyncio.sleep(1)
        logger.warning("[claude] URL monitor: never saw URL change to conversation URL")
    ...
    async def try_get_response(self) -> Optional[str]:
        ...
        if await self.is_generating():
            logger.info("[claude] try_get_response: still generating")
            return None
            ...
            try:
                ...
                if messages:
                    ...
                    if response and len(response) > 10:
                        logger.info(f"[claude] try_get_response: found response ({len(response)} chars)")
                        return response.strip()
            except:
                continue

        logger.info("[claude] try_get_response: no response found")
        return None
